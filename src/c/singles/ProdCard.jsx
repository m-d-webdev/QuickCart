...
export const RattingCmp = ({ product }) => {
    ...
    return (
        <div id={product.id} style={{ position: "relative" }} className='mt5 r-c-c'>
            ...
            {displayRatingVSBL && <DispayRatingCmp ref={rattingParef} Close_popup={() => { setdisplayRatingVSBL(false) }} />}
        </div>
    )
}
const ProductCard = React.forwardRef(({ product }, ref) => {
    ...
    const subStr_desc = (text) => {
        if (!isWorkingOnPC) {
            ...
            let Subed_text = text.substring(0, 100);
            Subed_text = `${Subed_text} ...`
            ...
            let Subed_text = text.substring(0, 150);
            Subed_text = `${Subed_text} ...`
